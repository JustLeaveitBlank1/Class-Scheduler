...
/**
 * Simple local slot type for helpers that need start/end strings.
 * This matches things like the old meetingSlots data without forcing a global type.
 */
type SimpleSlot = { start: string; end: string };

export function slotStartMin(slot: SimpleSlot | string) {
  const start = typeof slot === "string" ? slot : slot.start;
  return minutes(start);
}

export function slotEndMin(slot: SimpleSlot | string) {
  const end = typeof slot === "string" ? slot : slot.end;
  return minutes(end);
}

export const slotLen = (s: SimpleSlot) => slotEndMin(s) - slotStartMin(s);
